...
## B.9 Muteo y Conmutación de Fuentes (Hallazgo 2026-04-12)

### El problema del "Mute Hardware" (0x05)
En firmwares Nowada modernos, el comando de mute tradicional (`actionType=5` vía AIDL o broadcast `0x05`) es inestable. En algunas unidades, este comando provoca un **cambio de banda** inesperado (FM3) en lugar de silenciar, debido a que el MCU interpreta el "silencio de radio" como una señal para buscar portadoras alternativas o resetear el estado del sintonizador.

### Solución: Mute mediante "Source Swap"
La forma más robusta de mutear en QS6/NWD es mediante la conmutación de fuentes de audio del sistema:

1. **Silenciar (Mute true)**:
   - Cambiar fuente a **`SOURCE_ANDROID` (0x00)**.
   - Sincronizar claves de sistema para notificar al MCU que la radio no es la fuente activa:
     - `KEY_MCU_CURRENT_SOURCE = 0` (Android).
     - **`KEY_NWD_RADIO_BACK_SERVICE_ON = 0`** (Crítico: apaga la lógica de fondo del sintonizador en el kernel).
   - Emitir broadcast redundantes de cambio de fuente.

2. **Recuperar (Mute false)**:
   - Cambiar fuente a **`SOURCE_RADIO` (0x04)**.
   - Re-sincronizar claves:
     - `KEY_MCU_CURRENT_SOURCE = 4`.
     - `KEY_NWD_RADIO_BACK_SERVICE_ON = 1`.
   - Re-emitir handshake completo (`ACTION_APP_IN_OUT` con AppID 8).

### Claves de Registro (Settings.System) Involucradas

| Clave | Valor Mute | Valor Unmute | Propósito |
|-------|------------|--------------|-----------|
| `mcu_current_source` | `0` | `4` | Informa al MCU qué canal de audio físico abrir. |
| `nwd_radio_back_service_on` | `0` | `1` | Activa/Desactiva el driver de radio en el kernel. |

### ID Hallazgo: NWD-B004
**Hecho**: El MCU recupera el audio de radio automáticamente si detecta que la radio sigue en "back service", a pesar de haber pedido cambio a Android.
**Solución**: Forzar `nwd_radio_back_service_on = 0` durante el estado de mute.
