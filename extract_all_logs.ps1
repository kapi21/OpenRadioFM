...
$outFile = 'c:\@MIS PROYECTOS\K706_RE\OpenRadioFM\openradio_logs.txt'

$jsonObj = Get-Content $logFile -Raw | ConvertFrom-Json

$filtered = $jsonObj.logcatMessages | Where-Object { 
    $_.header.applicationId -eq 'com.example.openradiofm' -or $_.header.tag -match 'OpenRadioFm|AndroidRuntime|ActivityManager'
}
